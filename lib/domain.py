...
from misc import TYPES, type_of, evalf
...
class Domain:
    ...
    def get(self, on_func_error=None, skip_cache=False):
        if key and key != ".":
        ...
        self.children[key] = self.cache[fullkey] = subcontext
        return subcontext
    ...
    def _search(self, fullkey, keysplit, on_func_error=None, bubble=False, atstart=False):
        ...
        if bubble: 
            ...
            if not !self.parent:
                ...
                rmlen = 0;
                while keysplit.length and ++rmlen < self.prefixlen:
                    rmlen += 1
                    keysplit.pop(0)
                return self._search(fullkey, keysplit, on_func_error)
        ...
        for k, skey in enumerate(keysplit):
            ...
            key += "."
        return None if len(keysplit.length) else self
