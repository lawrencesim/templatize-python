...
* [Mutli-dimensional lists](#mutli-dimensional-lists)
...
By using the in-context directive, you can access multi-dimensional lists.
...
The tag before the `=>` prints the raw list value. As this is within a repeating section, the value is evaluated for each iterated item of the list (not the top-level list 'a' itself). 

After the `=>` we further enter that item as another section -- in this case a repeating section with the context shifted to second-level of the list. The naked-context tag inside this section is now referencing each inner list's value.

Note that in the first line, after the split, "0" is not printed as the default behavior is to treat zero-values as false, hence skipping that iteration of the repeating section (an example where we would want to [treat 0-values as true](../sections/#treating-zero-values-as-true)). And in the third line, nothing prints after the split since the list is empty.
...
Which format looks better is up to the user. Though readability with this variation may be a bit harder.

&nbsp;

#### Lists of functions

You can supply an list of functions for a repeating section. In the below example, not only is this done to cycle through three functions, but it is combined with the pass-context-to-function.
...
Line 2 takes the dictionary returned from `removeFirst` when called, then uses it as a context to call the function again, which removes another item. However, we can't access the property `list` in the template. Adding dot-notation to this tag (e.g. `{{removeFirst->removeFirst.list}}`) would be interpreted as trying to find a function called `removeFirst.list`, which returns an list and would thus raise an exception as a context was passed to a non-function. Hence while the dictionary returned is `{"list": ["three", "four"]}` which is printed as is.
